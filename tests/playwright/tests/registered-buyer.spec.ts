...
test.describe('Full end to end for Registered Buyers', () => {
  test("Placing and fulfilling a Registered Company's first Order", async ({
    ...
    verifyAddressDetails(
      testOrderData.billingAddress,
      hokodoOrder.customer.invoice_address
    );
    verifyAddressDetails(
      testOrderData.shippingAddress,
      hokodoOrder.customer.delivery_address
    );

    // ensure that one Organisation and One user was created for this Order
    const organisation = await hokodoApi.viewOrganisation(
      hokodoOrder.customer.organisation
    );
    ...
    if (
      getCaptureStatus(await magentoApi.getOrder(magentoOrderId)) ===
      MagentoOrderCaptureStatus.NotInvoiced
    ) {
    ...
    const deferredPayment = await hokodoApi.waitForDeferredPaymentToReachStatus(
      hokodoIds.deferredPayment,
      DeferredPaymentStatus.CAPTURED
    );
