...
export default class CreateAccountPage {
  ...
  async createAccount({ firstName, lastName, email, password }: Buyer) {
    ...
    if (firstName)
      await registrationForm.locator("[name='firstname']").fill(firstName);
    if (lastName)
      await registrationForm.locator("[name='lastname']").fill(lastName);
    ...
    if (password) {
      ...
      await registrationForm
        .locator("[name='password_confirmation']")
        .fill(password);
    }

    await this.page.waitForLoadState("networkidle");
    await registrationForm.locator("[type='submit']").click();
    await this.page.waitForSelector("text='Account Information'", {
      state: "attached",
    });
